  ...
  return (
    <form onSubmit={handleSearch} className="relative group w-full md:w-60">
      <SearchIcon className={cn(
          'absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 h-4 w-4 transition-colors duration-200 group-focus-within:text-blue-300',
          query ? 'text-blue-300' : ''
        )}
      />
      ...
      {query && (
        <CircleXIcon
          className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-300 h-4 w-4 cursor-pointer"
          onClick={resetSearch}
        />
      )}
    </form>
